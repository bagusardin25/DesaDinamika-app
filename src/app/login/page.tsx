...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    return () => {
      // Cleanup is optional; the script persists across navigations
    };
  }, []);
  ...
  // Pre-initialize the Google token client once the script loads
  const tokenClientRef = useRef<any>(null);

  useEffect(() => {
    if (!GOOGLE_CLIENT_ID) return;

    // Wait for the Google script then create the token client
    const initClient = () => {
  ...
  function handleGoogleLogin() {
    ...
    // requestAccessToken() MUST be called synchronously from the user click
    // to avoid popup blockers. That's why the token client is pre-initialized above.
    tokenClientRef.current.requestAccessToken();
  }


  // Don't render the form while redirecting an already-logged-in user
  if (user) return null;
      ...
      >
        ...
        <div className="flex items-center justify-center gap-3 mb-8">
          <div className="relative w-12 h-12 flex items-center justify-center rounded-xl overflow-hidden bg-[#18181b] shadow-lg shadow-violet-500/20 border border-violet-500/30">
            <Image src="/logo.svg" alt="DesaDinamika Logo" width={32} height={32} className="object-cover scale-[1.15]" priority />
          </div>
          <div className="flex items-center gap-2">
            <span className="font-bold text-2xl tracking-tight">DesaDinamika</span>
          </div>
        </div>
