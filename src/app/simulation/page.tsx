...
import { useDummySimulation } from "@/hooks/use-dummy-simulation";
...
function SimulationContent() {
  ...
  const isDemo = simId === "demo";

  const [showExitConfirm, setShowExitConfirm] = useState(false);
  const [soundEnabled, setSoundEnabled] = useState(true);

  const socketData = useSimulationSocket(isDemo ? null : simId, soundEnabled);
  const dummyData = useDummySimulation(soundEnabled);
  const sim = isDemo ? dummyData : socketData;
