    ...
    <aside className="w-[300px] border-l border-border bg-card/20 shrink-0 hidden lg:flex flex-col overflow-hidden">
      <div className="flex items-center justify-between p-4 pb-2 shrink-0">
        <h2 className="text-sm font-semibold flex items-center gap-2">
          <Activity className="w-4 h-4 text-primary" /> Indikator Desa
        </h2>
      ...
      <div className="flex-1 overflow-y-auto p-4 pt-2 space-y-3 custom-scroll">
        ...
        <Card className="bg-background/40 border-border/50 shadow-sm overflow-visible my-4">
          <CardHeader className="py-3 px-4 border-b border-border/20 bg-secondary/20">
            <CardTitle className="text-xs font-bold text-muted-foreground uppercase tracking-widest flex items-center gap-2">
              <Heart className="w-3.5 h-3.5" /> Kondisi Warga
            </CardTitle>
          </CardHeader>
          <CardContent className="p-4 pt-6 pb-10 flex items-center justify-around gap-2">
            <RadialGauge value={metrics.avgMorale} label="Kepuasan" color="text-emerald-500" size={72} />
            <RadialGauge value={metrics.avgStress} label="Konflik" color={metrics.avgStress > 70 ? "text-rose-500" : "text-amber-500"} size={72} />
            <RadialGauge value={metrics.productivity} label="Dukungan" color="text-blue-500" size={72} />
          </CardContent>
        </Card>
        ...
        <div className="grid grid-cols-2 gap-3">
          <MetricCard
            label="Solidaritas" value={metrics.teamCohesion}
            icon={Users} color="text-violet-500" delta={cohesionChange}
          />
          <MetricCard
            label="Kepercayaan" value={metrics.avgLoyalty}
        ...
        {metrics.resignations > 0 && (
          <Card className="bg-red-500/5 border-red-500/20 shadow-sm">
            <CardContent className="p-4 flex items-center gap-3">
              ...
              <div>
                <div className="text-xs text-muted-foreground font-medium">Protes Keras</div>
        ...
        {metricsHistory.length > 1 && (
          <Card className="bg-background/40 border-border/50 shadow-sm">
            <CardHeader className="py-3 px-4 border-b border-border/20">
              <CardTitle className="text-xs font-medium text-muted-foreground">Tren Historis</CardTitle>
            </CardHeader>
            <CardContent className="p-4 space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-[10px] text-muted-foreground">Kepuasan</span>
                <MiniSparkline data={moraleData} color="#22c55e" />
              </div>
              <div className="flex items-center justify-between">
                <span className="text-[10px] text-muted-foreground">Konflik</span>
            ...
            <CardContent className="p-4 space-y-3">
              <div className="space-y-2">
                <div className="flex items-center justify-between text-xs">
                  <span className="text-muted-foreground flex items-center gap-1"><DollarSign className="w-3 h-3" /> Anggaran</span>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between text-xs">
                  <span className="text-muted-foreground flex items-center gap-1"><Star className="w-3 h-3" /> Reputasi Kades</span>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between text-xs">
                  <span className="text-muted-foreground flex items-center gap-1"><Users className="w-3 h-3" /> Stabilitas</span>
              ...
              <div className="flex items-center justify-between text-xs pt-1 border-t border-border/20">
                <span className="text-muted-foreground flex items-center gap-1"><Clock className="w-3 h-3" /> Tenggat Waktu</span>
                <span className={`font-semibold ${worldState.deadlineWeeksLeft <= 2 ? "text-red-500" : "text-foreground"}`}>
                  {worldState.deadlineWeeksLeft}mgg tersisa
                </span>
              </div>
              <div className="flex items-center justify-between text-xs">
                <span className="text-muted-foreground">Risiko Sosial</span>
        ...
        {decisionStatus && (
          <Card className="bg-background/40 border-border/50 shadow-sm">
            <CardHeader className="py-3 px-4 border-b border-border/20">
              <CardTitle className="text-xs font-medium text-muted-foreground flex items-center gap-1.5">
                <MessageSquare className="w-3 h-3" /> Status Keputusan
              </CardTitle>
            </CardHeader>
            <CardContent className="p-4 space-y-2">
              {decisionStatus.hasDecision ? (
                ...
                <p className="text-xs text-muted-foreground">
                  {decisionStatus.proposalCount > 0
                    ? `${decisionStatus.proposalCount} usulan diajukan`
                    : "Belum ada usulan"}
                </p>
              )}
              {decisionStatus.resignThreats > 0 && (
                <div className="flex items-center gap-1.5 text-xs text-orange-400 pt-1">
                  ...
                  {decisionStatus.resignThreats} ancaman protes
                </div>
              )}
            </CardContent>
          </Card>
        ...
        {agents.length > 0 && (
          <Card className="bg-background/40 border-border/50 shadow-sm">
            <CardHeader className="py-3 px-4 border-b border-border/20">
              <CardTitle className="text-xs font-medium text-muted-foreground flex items-center gap-1.5">
                <Users className="w-3 h-3" /> Status Warga
              </CardTitle>
            </CardHeader>
        ...
        <div className="pt-2 space-y-3">
          <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Status Simulasi</h3>
          <div className="space-y-4 border-l-2 border-border/50 ml-2 pl-4 py-1">
            <div className="relative">
              ...
              <p className="text-xs text-muted-foreground font-medium mb-0.5">
                {status === "completed" ? "Selesai" : `Minggu ${currentRound} • Berjalan`}
              </p>
              <p className="text-sm">{status === "completed" ? "Semua iterasi telah selesai." : "Simulasi sedang berjalan."}</p>
            </div>
            <div className="relative opacity-60">
              <div className="absolute w-2 h-2 bg-primary/40 rounded-full -left-[21px] top-1.5 ring-4 ring-background" />
              <p className="text-xs text-muted-foreground font-medium mb-0.5">Inisialisasi</p>
              <p className="text-sm">Data Warga Dimuat</p>
            </div>
          </div>
        </div>
      </div>
