...
export function AgentSidebar({ agents, connectionError, typingAgentId }: AgentSidebarProps) {
  ...
  const getAgentState = (morale: number, stress: number) => {
    if (stress >= 85) return { label: "Terbakar", emoji: "🔥", color: "text-red-500 bg-red-500/10 border-red-500/20" };
    if (stress >= 65) return { label: "Tertekan", emoji: "😰", color: "text-orange-500 bg-orange-500/10 border-orange-500/20" };
    if (morale <= 40) return { label: "Frustrasi", emoji: "😡", color: "text-rose-500 bg-rose-500/10 border-rose-500/20" };
    if (morale >= 70 && stress <= 45) return { label: "Antusias", emoji: "✨", color: "text-emerald-500 bg-emerald-500/10 border-emerald-500/20" };
    return { label: "Aktif", emoji: "⚡", color: "text-blue-500 bg-blue-500/10 border-blue-500/20" };
  };
  return (
    <aside className="w-[300px] border-r border-border bg-card/20 p-4 hidden md:flex flex-col shrink-0 overflow-y-auto custom-scroll">
      <div className="space-y-1 mb-6">
        <h2 className="text-sm font-semibold flex items-center gap-2">
          <Users className="w-4 h-4 text-primary" /> Warga Virtual Aktif
        </h2>
        <p className="text-xs text-muted-foreground">Status psikologis real-time</p>
      </div>
                  ...
                  <div className="flex-1 space-y-2 min-w-0 z-10">
                    ...
                    {isTyping && (
                      ...
                      >
                        ...
                        <span className="text-[10px] text-primary font-medium">Sedang berpikir...</span>
                      </motion.div>
                    )}
          ...
          <div className="flex items-center justify-center h-40 text-muted-foreground text-sm">
            {connectionError ? (
              <div className="text-center space-y-2">
                ...
                <p className="text-xs text-orange-400">Koneksi terputus</p>
              </div>
            ) : (
              <>
                <Loader2 className="w-4 h-4 animate-spin mr-2" /> Memuat warga virtual...
              </>
            )}
          </div>
        )}
